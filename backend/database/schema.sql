...
CREATE INDEX IF NOT EXISTS idx_prod_crop_season ON production_foodgrains(crop, season, year);
CREATE INDEX IF NOT EXISTS idx_nfsa_state ON nfsa_coverage(state);
CREATE INDEX IF NOT EXISTS idx_stocks_state ON central_pool_stocks(state);
CREATE INDEX IF NOT EXISTS idx_proc_state_yr ON statewise_procurement(state, year);
