...
export default function StateIntelligencePage() {
  ...
  const { data: statesData, loading: statesLoading, error: statesError } = useAsync(() => api.getAnalyticsStates({ month: selectedMonth }), [selectedMonth]);
  const [state, setState] = useState("");
  const { data: detail, loading: detailLoading, error: detailError } = useAsync(() => state ? api.getAnalyticsState(state, { month: selectedMonth }) : Promise.resolve(null), [state, selectedMonth]);
  const states = statesData?.data || [];

  const kpis = useMemo(() => {
    if (!detail) return [];
    return [
      { label: `Central Pool Stock (${selectedMonth})`, value: fmt.num(detail.central_stock_lmt, 2), unit: "Lakh MT", icon: "fa-cubes", accent: "navy", foot: `FCI share ${fmt.pct(detail.fci_share_pct)}` },
      ...
      { label: `Upto ${selectedMonth} Offtake`, value: fmt.num(detail.upto_june_offtake_kt, 2), unit: "Thousand T", icon: "fa-truck", accent: "warn", foot: `${fmt.pct(detail.offtake_rate_pct)} of allocation` },
  ...
  if (statesLoading) return <Loading label={`Loading state intelligence index for ${selectedMonth} 2026…`} />;
  ...
  return <div>
    <PageHeader title={`State Intelligence — ${selectedMonth} 2026`} subtitle={`Consolidated state profiles for ${selectedMonth} 2026 assembled from NFSA, stock, procurement, allocation, offtake, welfare and paddy datasets.`} />
    ...
    {!state && <Panel title={`Choose a State / UT (${selectedMonth} 2026 Data)`} sub="The page combines multiple datasets into one decision-support profile."><div className="state-intro-grid">
      ...
      <div><Pill tone="navy">Distribution</Pill><p>Allocation, offtake, rate and remaining gap.</p></div>
    </div></Panel>}

    {state && detailLoading && <Loading label={`Building ${state} profile for ${selectedMonth} 2026…`} />}
    {state && detailError && <ErrorBox msg={detailError} />}
    {state && detail && <>
      <SectionTitle>{detail.state} — Key Indicators ({selectedMonth} 2026)</SectionTitle>
